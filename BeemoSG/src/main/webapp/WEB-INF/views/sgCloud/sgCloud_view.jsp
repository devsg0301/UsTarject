...
<script type="text/javascript">
	...
	function copy_trackback(trb) {
        ...
        if (IE) {
            if(confirm("복사하시겠습니까?"))
                window.clipboardData.setData("Text", "http://211.200.138.35:8081/LocalUser/data/" + trb);
        } else {
            temp = prompt("주소를 복사하세요", "http://211.200.138.35:8081/LocalUser/data/" + trb);
        }
    }
	...
	function fileDownload(fileUrl, fileName){
		...
		if(tmpUser.indexOf("iPhone") > 0 || tmpUser.indexOf("iPod") > 0){
			temp = prompt("iOS의 경우 아래주소를 복사하여 nPlayer(앱)의 브라우저탭에 주소를 넣고 이동하면 다운이 가능합니다.", "http://211.200.138.35:8081/LocalUser/data/" + fileUrl);
			return;
		}
                ...
                <c:if test="${broadcastDetail.category != 'UTILITY'}">
	                <div class="col-video">
		      			<video id="my-video" controls autoplay>
		      				<source src="http://211.200.138.35:8081/LocalUser/data/${broadcastDetail.file_url}" type='video/mp4'>
		      				<c:if test="${broadcastDetail.sub_url != ''}">
